...
contract AaveSaverTaker is DydxFlashLoanBase, ProxyPermission, GasBurner, DFSExchangeData {
    ...
    function repay(address _market, ExchangeData memory _data, uint256 _gasCost) public payable {
        _flashLoan(_market, _data, _gasCost, true);
    }

    function boost(address _market, ExchangeData memory _data, uint256 _gasCost) public payable {
        _flashLoan(_market, _data, _gasCost, false);
    }

    /// @notice Starts the process to move users position 1 collateral and 1 borrow
    /// @dev User must send 2 wei with this transaction
    function _flashLoan(address _market, ExchangeData memory _data, uint _gasCost, bool _isRepay) internal {
        ...
        operations[1] = _getCallAction(
            abi.encode(encodedData, _market, _gasCost, _isRepay, ethAmount, msg.value, proxyOwner(), address(this)),
            AAVE_RECEIVER
        );
