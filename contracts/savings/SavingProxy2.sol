...
contract SavingsProxy is ConstantAddresses {
    ...
    function depositAll(uint _amount) public {
        approveDeposit(SavingsProtocol.Compound, _amount);
        approveDeposit(SavingsProtocol.Dydx, _amount);
        approveDeposit(SavingsProtocol.Fulcrum, _amount);

        ProtocolInterface(getAddress(SavingsProtocol.Compound)).deposit(address(this), _amount / 3);
        ProtocolInterface(getAddress(SavingsProtocol.Dydx)).deposit(address(this), _amount / 3);
        ProtocolInterface(getAddress(SavingsProtocol.Fulcrum)).deposit(address(this), _amount / 3);

        endAction(SavingsProtocol.Dydx);
    }

    function withdrawAll(uint _amount) public {
        approveWithdraw(SavingsProtocol.Compound, _amount);
        approveWithdraw(SavingsProtocol.Dydx, _amount);
        approveWithdraw(SavingsProtocol.Fulcrum, _amount);

        ProtocolInterface(getAddress(SavingsProtocol.Compound)).withdraw(address(this), _amount / 3);
        ProtocolInterface(getAddress(SavingsProtocol.Dydx)).withdraw(address(this), _amount / 3);
        ProtocolInterface(getAddress(SavingsProtocol.Fulcrum)).withdraw(address(this), _amount / 3);
