...
describe("Aave-Monitor", function() {
    ...
    const supplyAmount = hre.ethers.utils.parseUnits("30", "18");
    const withdrawAmount = hre.ethers.utils.parseUnits("15", "18");

    const boostAmount = hre.ethers.utils.parseUnits("5000", "6");
    ...
    it(`... should preform an automatic repay for aave position: ${aavePositionAddr}`, async () => {
        ...
        // const ratioBefore = await aaveLoanInfoV2.getRatio(proxy.address);
        // console.log(`Ratio before repay ${ratioBefore / 1e16}`);

        // // send tx
        // await aaveMonitorV2.repayFor(exchangeData, [cEthAddr, cUsdcAddr], { gasLimit: 8000000 });

        // const ratioAfter = await aaveLoanInfoV2.getRatio(proxy.address);
